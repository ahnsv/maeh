...
Use `maeh --help` as the command index. The help output is intentionally short,
line-oriented, and safe to pass to an agent. When a narrower surface exists,
prefer the subcommand help (`maeh state --help`) or the command contract in
`docs/bash-helper-parity.md` before reading source.
